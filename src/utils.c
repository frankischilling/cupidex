...
void draw_scrolling_banner(WINDOW *window, const char *text, const char *build_info, int offset) {
    ...
    time_t rawtime;
    struct tm *timeinfo;
    char time_str[9];  // HH:MM:SS format
    ...
    int total_len = width + text_len + build_len + time_len + 6;
    ...
    for (int i = 0; i < 2; i++) {
        int pos = i * total_len;
        snprintf(scroll_text + pos, total_len, "%s | %s | %s", text, build_info, time_str);
    }
...
/** Function to cleanup tmp files made by operations
*/
...
/** Function to return if file is hidden
