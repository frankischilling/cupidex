...
void get_desktop_environment() {
    ...
    const char* xSessionType = getenv("xSessionType");
    if (xSessionType != NULL && strlen(xSessionType) > 0) {
        print_info("DE", xSessionType, 20, 30);
        return; // Successfully retrieved
    }

    // NOTE: hardcoded :(

    FILE* processFile = popen("ps -e | grep -Eo '^[0-9]+\\s+\\w+' | awk '{print $2}'", "r");
    if (processFile != NULL) {
        char processName[50];
        while (fgets(processName, sizeof (processName), processFile) != NULL) {
            strtok(processName, "\n");
            // check for env
            if (strstr(processName, "gnome-shell") != NULL) {
